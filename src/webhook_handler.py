...
async def proxy_redirect(request):
    return web.HTTPFound("tg://proxy?server=2.26.73.202&port=8443&secret=eed5e6f7a8b9c0d1e2f3a4b5c6d7e8f9a07275747562652e7275")
...
async def get_subscription(request):
    try:
        ...
        with Session() as db_session:
            ...
            if not u:
                return web.Response(text=f"Subscription not found (UID {user_id} not matched in DB)", status=404)
        ...
        with Session() as session:
            ...
            if u:
                if act == "custom_days":
                    ...
                    return web.json_response({"status": "ok", "msg": "Карта успешно отвязана!"})
